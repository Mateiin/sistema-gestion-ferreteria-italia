...
export class Comprobante {
  ...
  /**
   * Information Expert: calcula neto e IVA de una línea. `precioUnitario` es
   * siempre NETO (sin IVA) — así es como se carga en la ficha, sea Factura A
   * o B: se SUMA el IVA en los dos casos. No redondea: el redondeo se hace
   * una sola vez, por alícuota ya agrupada (ver `calcularDesglose`), para no
   * arrastrar error de redondeo línea a línea.
   */
   ...
   * `subtotalNeto` es el precio cargado (ya es neto) multiplicado por la
   * cantidad.
   */
  static armarDetalle(
